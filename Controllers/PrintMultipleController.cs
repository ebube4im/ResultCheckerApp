...
namespace GodlySeeds_Online_Result_Checker.Controllers
{
    ...
    {
        public FileInfo myfile { get; private set; }
        ...
        public ActionResult PrintMultiple(PrintResultViewModel printmodel)
        {    
            //classToPrint = "";
            List<PrintResultViewModel> classResultList = new List<PrintResultViewModel>();


            switch (printmodel.Class)
            {
                case "JSS 1A":
                    myfile = new FileInfo(Server.MapPath("~/Content/Uploads/1stTerm/JSS1A.xlsx"));
                    break;
                case "JSS 1B":
                    myfile = new FileInfo(Server.MapPath("~/Content/Uploads/1stTerm/JSS1B.xlsx"));
                    break;
                case "JSS 2A":
                    myfile = new FileInfo(Server.MapPath("~/Content/Uploads/1stTerm/JSS2A.xlsx"));
                    break;
                case "JSS 2B":
                    myfile = new FileInfo(Server.MapPath("~/Content/Uploads/1stTerm/JSS2B.xlsx"));
                    break;
                case "JSS 3A":
                    myfile = new FileInfo(Server.MapPath("~/Content/Uploads/1stTerm/JSS3A.xlsx"));
                    break;
                case "JSS 3B":
                    myfile = new FileInfo(Server.MapPath("~/Content/Uploads/1stTerm/JSS3B.xlsx"));
                    break;
                case "SSS 1A":
                    //mymodel.ErrorMessage = "Oops! No result has been uploaded yet for the selected class. Please check back later";
                    //return View("Index", mymodel);
                    myfile = new FileInfo(Server.MapPath("~/Content/Uploads/1stTerm/SSS1A.xlsx"));
                    break;
            ...
            if (myfile != null)
            {
                using (var package = new ExcelPackage(myfile))
                {
                    ...
                    foreach (var item in AdmissionNo)
                    {
                        //int number = AdmissionNo.IndexOf(item) + 3;
                        int number = AdmissionNo.IndexOf(item) + 3;
